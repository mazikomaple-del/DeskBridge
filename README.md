...
実際の日本語画面です（撮影版v1.0.160、現在の配布版v1.0.166）。2台のPCカードを上下に配置した例です。通常はそれぞれの画面を見ながら操作先を切り替え、必要なときはPCカードのダブルクリックで相手の画面を開けます。
...
1.0.166でリモート画面表示を復活しました。利用する両方のPCを1.0.166以上に更新してください。UAC確認画面は、そのPCで直接操作する必要があります。PCごとの接続状態表示と4言語の初回ガイドも追加しています。DB3ライセンスをお使いの場合、更新後の初回起動時はインターネットへ接続してください。
